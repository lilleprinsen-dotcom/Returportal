...
final class LP_Cargonizer_Returns {
    ...
    public function admin_menu(){
        ...
        add_menu_page('Cargonizer Retur','Cargonizer Retur','manage_woocommerce','lp-cargo-returns',[$this,'settings_page'],'dashicons-rewind',56);
        add_submenu_page('lp-cargo-returns','Returlogg','Returlogg','manage_woocommerce','lp-cargo-returns-log',[$this,'render_returns_log_page']);
    }

    public function register_settings(){
        // Kjerne
        register_setting('lp_cargo_settings', self::OPT_API_KEY, [
            'sanitize_callback' => function($v){
                $v = trim((string)$v);
                if ($v==='') return get_option(self::OPT_API_KEY,'');
                return sanitize_text_field($v);
            }
        ]);
        register_setting('lp_cargo_settings', self::OPT_SENDER_ID,     ['sanitize_callback'=>'sanitize_text_field']);
        register_setting('lp_cargo_settings', self::OPT_AUTO_TRANSFER, ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_ATTACH_PDF,    ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_SWAP_PARTIES,  ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_EMAIL_VIA_LOG, ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_TT_NOTIFY,     ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_DEFAULT_SERV,  ['sanitize_callback'=>function($v){
            if (!is_array($v)) return [];
            foreach($v as $k=>&$arr){ $arr = array_values(array_unique(array_map('sanitize_text_field',(array)$arr))); }
            return $v;
        }]);
        register_setting('lp_cargo_settings', self::OPT_ALLOWED,       ['sanitize_callback'=>function($v){
            return array_values(array_unique(array_map('sanitize_text_field',(array)$v)));
        }]);
        register_setting('lp_cargo_settings', self::OPT_FEE_SMALL,     ['sanitize_callback'=>function($v){ return (string)max(0,(int)$v); }]);
        register_setting('lp_cargo_settings', self::OPT_FEE_LARGE,     ['sanitize_callback'=>function($v){ return (string)max(0,(int)$v); }]);
        register_setting('lp_cargo_settings', self::OPT_RETURN_WINDOW, ['sanitize_callback'=>function($v){ return (string)max(0,(int)$v); }]);

        // FS + bannerfarge
        register_setting('lp_cargo_settings', self::OPT_FS_BONUS_ENABLE, ['sanitize_callback'=>function($v){ return $v==='1'?'1':'0'; }]);
        register_setting('lp_cargo_settings', self::OPT_FS_BONUS_HOURS,  ['sanitize_callback'=>function($v){ return (string)max(1,(int)$v); }]);
        register_setting('lp_cargo_settings', self::OPT_FS_BANNER_COLOR, ['sanitize_callback'=>function($v){
            $v = trim((string)$v); if ($v==='') return '#0ea5e9';
            return preg_match('/^#([A-Fa-f0-9]{6})$/',$v) ? $v : '#0ea5e9';
        }]);

        // Label gyldighet/retensjon
        register_setting('lp_cargo_settings', self::OPT_LABEL_VALID_DAYS,     ['sanitize_callback'=>function($v){ return (string)max(7,(int)$v); }]);
        register_setting('lp_cargo_settings', self::OPT_LABEL_RETENTION_DAYS, ['sanitize_callback'=>function($v){ return (string)max(7,(int)$v); }]);

        // Support-epost, årsaker, bytte-info
        register_setting('lp_cargo_settings', self::OPT_SUPPORT_EMAIL, ['sanitize_callback'=>'sanitize_email']);
        register_setting('lp_cargo_settings', self::OPT_RETURN_REASONS, ['sanitize_callback'=>function($v){
            if (is_array($v)) $lines = $v; else $lines = preg_split('/\r\n|\r|\n/', (string)$v);
        ...
        register_setting('lp_cargo_settings', self::OPT_EXCHANGE_INFO, ['sanitize_callback'=>function($v){
            $v = trim((string)$v); return $v!=='' ? wp_kses_post($v) : 'Ønsker du bytte? Vi dekker frakt på ny forsendelse.';
        }]);

        // Defaults (autoload)
        if (get_option(self::OPT_AUTO_TRANSFER,'')==='') update_option(self::OPT_AUTO_TRANSFER,'0',true); // default OFF
        if (get_option(self::OPT_ATTACH_PDF,'')  ==='') update_option(self::OPT_ATTACH_PDF,'0',true);
        if (get_option(self::OPT_SWAP_PARTIES,'')==='') update_option(self::OPT_SWAP_PARTIES,'0',true);
        if (get_option(self::OPT_EMAIL_VIA_LOG,'')==='') update_option(self::OPT_EMAIL_VIA_LOG,'1',true);
        if (get_option(self::OPT_TT_NOTIFY,'')   ==='') update_option(self::OPT_TT_NOTIFY,'0',true);
        if (get_option(self::OPT_DEFAULT_SERV,'')==='') update_option(self::OPT_DEFAULT_SERV,[],false);
        if (get_option(self::OPT_ALLOWED,null)===null) update_option(self::OPT_ALLOWED,[],false);
        if (get_option(self::OPT_FEE_SMALL,'')  ==='') update_option(self::OPT_FEE_SMALL,'69',true);
        if (get_option(self::OPT_FEE_LARGE,'')  ==='') update_option(self::OPT_FEE_LARGE,'129',true);
        if (get_option(self::OPT_RETURN_WINDOW,'')==='') update_option(self::OPT_RETURN_WINDOW,'30',true);

        if (get_option(self::OPT_FS_BONUS_ENABLE,'')==='') update_option(self::OPT_FS_BONUS_ENABLE,'1',true);
        if (get_option(self::OPT_FS_BONUS_HOURS,'')==='')  update_option(self::OPT_FS_BONUS_HOURS,'24',true);
        if (get_option(self::OPT_FS_BANNER_COLOR,'')==='') update_option(self::OPT_FS_BANNER_COLOR,'#0ea5e9',true);

        if (get_option(self::OPT_LABEL_VALID_DAYS,'')==='')     update_option(self::OPT_LABEL_VALID_DAYS,'14',true);
        if (get_option(self::OPT_LABEL_RETENTION_DAYS,'')==='') update_option(self::OPT_LABEL_RETENTION_DAYS,'30',true);

        if (get_option(self::OPT_SUPPORT_EMAIL,'')==='') update_option(self::OPT_SUPPORT_EMAIL, get_option('admin_email'), true);
        if (get_option(self::OPT_EXCHANGE_INFO,'')==='') update_option(self::OPT_EXCHANGE_INFO,'Ønsker du bytte? Vi dekker frakt på ny forsendelse.',true);
        if (get_option(self::OPT_RETURN_REASONS,'')==='') update_option(self::OPT_RETURN_REASONS, ['Feil størrelse','Ikke som forventet','Defekt / varefeil','Angrer kjøp','Annet'], false);

        // API-key autoload=no
        $val = get_option(self::OPT_API_KEY, null);
        if ($val === null) add_option(self::OPT_API_KEY, '', '', 'no'); else update_option(self::OPT_API_KEY, $val, false);
    }

    public function settings_page(){
        if (!current_user_can('manage_woocommerce')) return;

        $nonce_fetch = wp_create_nonce(self::NONCE);
        $api     = get_option(self::OPT_API_KEY,'');
        $sender  = get_option(self::OPT_SENDER_ID,'');
        $feeS    = get_option(self::OPT_FEE_SMALL,'69');
        $feeL    = get_option(self::OPT_FEE_LARGE,'129');
        $window  = get_option(self::OPT_RETURN_WINDOW,'30');
        $fsOn    = get_option(self::OPT_FS_BONUS_ENABLE,'1');
        $fsHrs   = get_option(self::OPT_FS_BONUS_HOURS,'24');
        $fsCol   = get_option(self::OPT_FS_BANNER_COLOR,'#0ea5e9');
        $labelV  = get_option(self::OPT_LABEL_VALID_DAYS,'14');
        $labelR  = get_option(self::OPT_LABEL_RETENTION_DAYS,'30');
        $support = get_option(self::OPT_SUPPORT_EMAIL, get_option('admin_email'));
        $reasons = (array)get_option(self::OPT_RETURN_REASONS, []);
        $exInfo  = get_option(self::OPT_EXCHANGE_INFO,'');
        ...
        // API/key
        $hasKey = $api !== '';
        echo '<tr><th>API-nøkkel (X-Cargonizer-Key)</th><td><input type="password" name="'.self::OPT_API_KEY.'" value="" autocomplete="off" style="width:420px" placeholder="'.($hasKey?'••••••••':'').'"><p class="description">'.($hasKey?'La felt tom for å beholde eksisterende nøkkel.':'Lim inn nøkkel.').'</p></td></tr>';
        echo '<tr><th>Avsender-ID (X-Cargonizer-Sender)</th><td><input type="text" name="'.self::OPT_SENDER_ID.'" value="'.esc_attr($sender).'" style="width:320px"></td></tr>';
        ...
        // New: Auto-transfer toggle
        echo '<tr><th>Overfør direkte til transportør</th><td><label><input type="checkbox" name="'.self::OPT_AUTO_TRANSFER.'" value="1" '.checked(get_option(self::OPT_AUTO_TRANSFER,'0'),'1',false).'> Overfør konsignasjonen umiddelbart ved opprettelse</label><p class="description">Hvis aktivert opprettes konsignasjonen og overføres deretter via <code>/consignments/transfer</code>. Som standard overføres ikke konsignasjonen ved opprettelse.</p></td></tr>';

        // Bytt parter
        echo '<tr><th>Bytt avsender/mottaker</th><td><label><input type="checkbox" name="'.self::OPT_SWAP_PARTIES.'" value="1" '.checked(get_option(self::OPT_SWAP_PARTIES,'0'),'1',false).'> Bruk kun om transportøren krever omvendt part-oppsett</label></td></tr>';

        echo '<tr><th>Returavgift</th><td>Liten (kr) <input type="number" name="'.self::OPT_FEE_SMALL.'" value="'.esc_attr($feeS).'" style="width:100px"> &nbsp; Stor (kr) <input type="number" name="'.self::OPT_FEE_LARGE.'" value="'.esc_attr($feeL).'" style="width:100px"></td></tr>';
        echo '<tr><th>Returfrist (dager)</th><td><input type="number" name="'.self::OPT_RETURN_WINDOW.'" value="'.esc_attr($window).'" style="width:100px"></td></tr>';
        echo '<tr><th>Etikett gyldig (dager)</th><td><input type="number" name="'.self::OPT_LABEL_VALID_DAYS.'" value="'.esc_attr($labelV).'" style="width:100px"></td></tr>';
        echo '<tr><th>Slett etiketter etter (dager)</th><td><input type="number" name="'.self::OPT_LABEL_RETENTION_DAYS.'" value="'.esc_attr($labelR).'" style="width:100px"></td></tr>';

        echo '<tr><th>Fraktfri i 24t</th><td><label><input type="checkbox" name="'.self::OPT_FS_BONUS_ENABLE.'" value="1" '.checked($fsOn,'1',false).'> Aktiver fraktfri neste ordre</label><br>Tidsvindu (timer) <input type="number" name="'.self::OPT_FS_BONUS_HOURS.'" value="'.esc_attr($fsHrs).'" style="width:100px"> &nbsp; Bannerfarge <input type="text" name="'.self::OPT_FS_BANNER_COLOR.'" value="'.esc_attr($fsCol).'" placeholder="#0ea5e9" style="width:120px"></td></tr>';

        echo '<tr><th>Kundeservice-epost</th><td><input type="email" name="'.self::OPT_SUPPORT_EMAIL.'" value="'.esc_attr($support).'" style="width:320px"></td></tr>';

        echo '<tr><th>Returårsaker</th><td><textarea name="'.self::OPT_RETURN_REASONS.'" rows="5" style="width:420px">'.esc_textarea(implode("\n",$reasons)).'</textarea><p class="description">Én per linje. «Annet» legges til automatisk hvis ikke oppgitt.</p></td></tr>';

        echo '<tr><th>Info i start</th><td><textarea name="'.self::OPT_EXCHANGE_INFO.'" rows="3" style="width:520px">'.esc_textarea($exInfo).'</textarea><p class="description">Vises i steg 1 (f.eks. «Ønsker du bytte? Vi dekker frakt på ny forsendelse.»)</p></td></tr>';

        echo '<tr><th>Tilgjengelige tjenester</th><td>
                <button class="button" id="lp-cargo-refresh">Last avtaler</button>
                <div id="lp-cargo-agreements" style="margin-top:10px"></div>
                <p class="description" style="margin-top:6px">Huk av produkter som skal vises i returportalen. Klikk «Lagre valg» for å lagre.</p>
              </td></tr>';

        // Admin unlock
        echo '<tr><th>Overstyr retur-lås</th><td><input type="text" id="lp_unlock_order_id" placeholder="#1234" style="width:120px"> <button type="button" class="button" id="lp_unlock_btn">Lås opp</button></td></tr>';
        ...
        // Agreements UI CSS
        $css = '.lp-serv-wrap{margin-top:6px;padding:8px;border:1px dashed #d0d0d0;border-radius:8px;background:#fafafa}.lp-serv-title{font-weight:600;margin:0 0 6px}.lp-small{color:#6b7280;font-size:12px}details>summary{cursor:pointer}';
        echo '<style>'.esc_html($css).'</style>';

        // Admin-side JS – eksplisitt admin-ajax URL
        $ajax = <<<'JS'
(function(){
 const box=document.getElementById('lp-cargo-agreements');
 const btn=document.getElementById('lp-cargo-refresh');
 const nonce='%NONCE%';
 const AJ=(typeof ajaxurl!=='undefined' && ajaxurl) ? ajaxurl : '%AJAX%';
 let dirty=false;

 async function post(action, payload){
   const form=new URLSearchParams(); form.append('action',action); form.append('_wpnonce',nonce);
...
JS;
        ...
        $ajax = str_replace('%OPT_API_KEY%', self::OPT_API_KEY, $ajax);
        $ajax = str_replace('%OPT_SENDER_ID%', self::OPT_SENDER_ID, $ajax);
